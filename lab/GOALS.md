...
- **Current (Session 47, 2026-06-09)**: **N≥30 gate CLEARED → N=108 resolved markets. Verdict: NO EDGE.** `eval/resolution_backtest.py` (scored vs actual Polymarket resolution, no longer underpowered): **directional 63.9% (69/108, 95% CI 54.5–72.3%)** — but avg entry price is **0.636**, so **edge vs price-implied is +0.3 pp** = indistinguishable from the market's own prices. The 63.9% recovers Polymarket's prior; it does not beat it (all calls Bullish on already-high-priced markets). Friction-adjusted paper win rate **10.2% (961W/8431L)**. **Decision: HOLD the dominoes** — do NOT lift the Bearish ban, retrain XGBoost, or enable META_GATE; there is no edge to amplify. `AUTO_RETRAIN_ENABLED` = false. **Next frontier = a price-orthogonal signal** (news features Tier 5.3 / whale-microstructure / cross-market) that can clear >+0.3pp edge-vs-price. **Target unchanged**: live $1 trade gate stays closed until a backtest shows real edge-vs-price AND friction-adjusted win rate ≥45%.
...
|--------|---------|--------|---------|----------|
| Prediction Accuracy | **48% overall** (305W/330L). 4h directional: 44.9%. Session 39: threshold fix shows 60.5% potential — recovering. | 60% | 70% | 80% |
| Trades Executed | 0 (7,109 paper — approval gate proven) | 10 | 100 | 1000 |
| P&L (USDC) | $0 (paper: 5947W/996L overall, last 100: 47%) | +$5 | +$100 | +$1000 |
| API Costs/month | **~$5** (Claude Sonnet heartbeats, $30 balance) | $25 | $20 | $15 |
| Loop Autonomy | **7/10** (Sonnet heartbeats, real tool calls, real-time comms via openclaw agent CLI) | 8/10 | 9/10 | 9/10 |
| GPU Utilization | ~0% idle, spikes when Ollama loads | 20% | 50% | 80% |
