...
def derive_time_horizon(
    ...
    if effective_delta > 0.10 or volume_24h > 5_000_000:
        return "1h"
    elif effective_delta > 0.05 or volume_24h > 1_000_000:
        return "4h"
...
def _run_tests():
    ...
    # Test 2: Medium activity → 4h
    print("\n[Test 2] Medium activity")
    assert_test("Medium delta → 4h", derive_time_horizon(500_000, 0.07) == "4h")
    assert_test("Medium volume → 4h", derive_time_horizon(2_000_000, 0.01) == "4h")
    ...
    print("\n[Test 6] Boundaries")
    assert_test("Exact 0.10 → 1h", derive_time_horizon(0, 0.10) == "4h")  # >0.10, not >=
    assert_test("Just over 0.10 → 1h", derive_time_horizon(0, 0.101) == "1h")
    assert_test("Exact 5M vol → 4h", derive_time_horizon(5_000_000, 0.0) == "4h")  # >5M, not >=
